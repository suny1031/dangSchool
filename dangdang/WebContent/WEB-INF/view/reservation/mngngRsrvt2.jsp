...
<%@page import="com.dang.member.user.model.vo.UserMember"%>
...
<link rel="stylesheet" href="/resources/css/mngngRsrvt2.css" />
...
<!--  -->
 ...
 <%
	...
	UserMember userId = (UserMember) session.getAttribute("userMember");
	...
	count = reservationService.selectCountPageUser(userId.getUserId()); // 데이터베이스에 저장된 총 갯수


 	List<Reservation> list = null;
	if (count > 0) {
		// getList()메서드 호출 / 해당 레코드 반환
		list = reservationService.selectReservationUserPage(startRow, endRow, userId.getUserId());
		...
		<table id = "table" align="center">
			...
			<tr align="center">
				...
				<td><%=reservation.getDogAge()%></td>
					...
					<td><div class= "approvedBtn">승인전</div></td>
					<%}else {%>
			...
			<tr>
				<td align="center" colspan="11" style="font-size: 0.7vw">
					...
					%>
								<a href="/reservation/mngngRsrvt2.do?pageNum=<%=startPage - 10%>">[이전]</a>	
					<%			
							}
					...
					%>
									<a href="/reservation/mngngRsrvt2.do?pageNum=<%=i%>">[<%=i %>]</a>
					<%	
								}
							} // for end
					...
					%>
								<a href="/reservation/mngngRsrvt2.do?pageNum=<%=startPage + 10 %>">[다음]</a>
					<%			
							}
						}
					%>
